...
function isPositiveNumber(num) {
  ...
  } else { // Якщо так, то num є позитивним, тому повертаємо true
    return false; 
...
function isEmptyString(str) {
  if (str.trim() == '') {
    return true;// Перевіряємо, чи рядок str дорівнює пустому рядку ('')
  } else { // Якщо так, то str є порожнім рядком, тому повертаємо true
    return false;
...
function getUserStatus(age) {
  if (age < 18) {
    return "Неповнолітній" // Перевіряємо вік за допомогою умовних операторів
  } else if (age >= 18 && age < 65) { // Якщо age менше 18, то користувач є "Неповнолітнім"
...
function getDayOfWeek(dayNumber) {
  let day; // Використовуємо оператор switch для знаходження назви дня тижня залежно від dayNumber
...
function getVariableType(variable) {
  ...
  const message = // Встановлюємо відповідне повідомлення залежно від отриманого типу
    VariableType === "number" // Якщо тип змінної - number, то повідомлення = "Це число"
      ? "Це число" 
      : VariableType === "string"
